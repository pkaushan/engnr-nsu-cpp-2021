...
template<typename T, typename Allocator>
template<typename Compare>
void LinkedList<T, Allocator>::sort_(iterator begin_, iterator end_, Compare comp) {

    auto num_elements = std::distance(begin_, end_);
    if (num_elements <= 1){
        return;
    }
    ...
    if (num_elements > 2){
        ...
        auto middle = begin_;
        std::advance(middle, num_elements/2);
    ...
    sort_(begin_,  left, comp);
    sort_(++left, end_, comp);
}


template<typename T, typename Allocator>
template<typename Compare>
void LinkedList<T, Allocator>::sort(Compare comp) {

    sort_(begin(), end(), comp);
}


template<typename T, typename Allocator>
void LinkedList<T, Allocator>::sort() {

    sort_(begin(), end(), std::less<value_type>{});
}
